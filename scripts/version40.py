...
# ACTION_SEND is handled from onResume so it works both on a cold start and
# when Rutt GPS is already open, without depending on older onCreate layouts.
anchor = ' private void chooseFile(){'
if anchor not in s:
    raise SystemExit('v40 chooseFile point not found')
helper = r''' @Override protected void onResume(){super.onResume();handleSharedRouteIntent(getIntent());}
 @Override protected void onNewIntent(Intent intent){super.onNewIntent(intent);setIntent(intent);handleSharedRouteIntent(intent);}
 ...
 private void showSharedGoogleMapsImport(String text){final String[]shareDays={"Måndag","Tisdag","Onsdag","Torsdag","Fredag"};int start=0;for(int i=0;i<shareDays.length;i++)if(shareDays[i].equals(selectedDay)){start=i;break;}final int[]pick={start};new androidx.appcompat.app.AlertDialog.Builder(this).setTitle("Importera Google Maps-rutt").setMessage("Välj vilken dag rutten ska sparas på.").setSingleChoiceItems(shareDays,pick[0],(d,w)->pick[0]=w).setNegativeButton("Avbryt",null).setPositiveButton("Importera",(d,w)->{selectedDay=shareDays[pick[0]];importGoogleMaps(text);}).show();}
'''
